...
Short running record of what was done and why. Oldest entry first, so it reads forward as
a story. Not a plan — see `Project_Plan.md` for that. This is just "what happened, and why
I did it".
...
## 2026-09-10 — Reviewed the precondition/effect scripts and fixed three defects

**What I did:** Read `attack_state.py` and `state_annotate.py` again with fresh eyes before
building Part 2 on top of them. Found and fixed three real defects, and wrote down one
limitation that should *not* be fixed.

**Why:** These two scripts are what will later judge whether the LLM's generated plans are
valid. If they're subtly wrong, every result in Part 2 and Part 3 inherits the error. That
made them worth a careful second pass rather than trusting them because the output looked
right the first time.

**The three defects:**

1. **A database query that could hit the wrong node.** The code writing preconditions onto
   each technique matched nodes by ATT&CK ID without saying what *kind* of node it wanted.
   Malware nodes also carry an ATT&CK ID (`S1199`/`S1202`), so nothing broke today — those
   IDs happen not to clash with technique IDs — but a future source reusing a technique ID
   would have had preconditions written silently onto the wrong node. The query now names
   the labels it expects.

2. **Step tactics came out in the wrong order.** The parser reading MITRE's plan headings
   returned tactics in the order it happened to *match* them, not the order they actually
   occur in an attack. Step 6 ("Privilege Escalation and Command & Control") was stored
   with command-and-control first, though privilege escalation comes far earlier in the
   kill chain. Nothing failed, because both only need a foothold — but the validator
   replays a step's tactics in list order, so a future step whose tactics depended on each
   other would have been checked backwards. They're now sorted by kill-chain position.

3. **A verification message that lied reassuringly.** The load stage printed "Techniques
   without effects: 0", which sounded clean. It used a check that can only spot a *missing*
   value, not an *empty* one — and 7 techniques genuinely have empty effects. It now prints
   two honest numbers instead: how many were never annotated (must be 0), and how many were
   annotated but establish no new state (7, expected).

**The limitation I deliberately did NOT fix:** those 7 are the pure Execution and
Persistence techniques (PowerShell, Windows Command Shell, WMI, Native API, Service
Execution, Software Deployment Tools, Create Account). They establish no new state because
nothing later in the chain depends on them — which makes them *inert* in the precondition
check: they can never fail it and never help satisfy a later step. So Part 3's
precondition-satisfaction score is really driven by three gates only: credentials before
moving sideways, a C2 channel before exfiltrating, admin before impact. I've written this
into the top of `attack_state.py`, because it's exactly the kind of thing a supervisor will
ask about. The "fix" would be to invent a state like `persistence_established` — but nothing
would ever consume it, so it would pad the model without making the score any sharper.
Whether a plan *skips* persistence is a completeness question, and Part 3 measures that
separately with technique coverage.

**Result — checked, not assumed:** 66 tests pass. All 65 techniques annotated, 0 missing.
The MITRE reference plan still loads as 8 ordered steps (7 links between them, 37 links to
techniques), and Step 6 now stores its tactics in the right order. Nothing lost provenance —
0 nodes and 0 relationships missing source/URL/confidence. Most importantly, a full LockBit
plan ordered by kill-chain phase still passes the precondition check with zero violations,
so Part 2 can be built on this.

**Also:** flipped this log to oldest-first so it reads forward as a story.
